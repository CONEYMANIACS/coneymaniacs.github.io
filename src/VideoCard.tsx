...
// function chaptersDict(chapterTitle: string){

//   if(chaptersMap.has(chapterTitle)){
//     let value = chaptersMap.get(chapterTitle)
//     chaptersMap.set(chapterTitle, value+1);
//   }
//   else{
//     chaptersMap.set(chapterTitle, 1);
//   }
//   if(chaptersMap.size > 1838){
//     console.log("RUNNING")

//     const entries = Array.from(chaptersMap, ([name, data]) => ({ name, data }));

//     const fuse = new Fuse(entries, {
//   keys: ['name'],
//   threshold: 0.8 // Lower = stricter match
// });

// const reducedMap = entries.reduce((acc, current) => {
//   const matches = fuse.search(current.name);
  
//   if (matches.length > 0) {
//     const bestMatch = matches[0].item.name;
    
//     // Only add if we haven't already processed this fuzzy cluster
//     if (!acc.has(bestMatch)) {
//       acc.set(bestMatch, current.data);
//     }
//   }
//   return acc;
// }, new Map());
...
// }
//href={chapterLink(chapters.timestamp, video.vidId)}